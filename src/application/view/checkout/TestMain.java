...
public class TestMain extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			//Parent root = FXMLLoader.load(getClass().getResource("/application/view/checkout/CheckoutForLibMem.fxml"));
			Scene scene = new Scene(root,820,720);
			//scene.getStylesheets().add(getClass().getResource("application.css").toExternalForm());
			//CheckoutForLirMemController.initMemberList(primaryStage);
			primaryStage.setScene(scene);
			primaryStage.setResizable(false);//window size immutable
			primaryStage.setTitle("CheckoutForLibraryMember");
			primaryStage.show();
			...
			for (Node circle : circles.getChildren()) {
				timeline.getKeyFrames().addAll(
						...
						new KeyFrame(new Duration(10000), // set end position at 40s
								new KeyValue(circle.translateXProperty(),
										Math.random() * 1000), new KeyValue(circle
										.translateYProperty(), Math.random() * 1000)));
			}
	...
	public static void main(String[] args) {
		//launch(args);
		Application.launch(TestMain.class);
	}
}